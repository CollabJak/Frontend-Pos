...
export default function SignInForm() {
  const { login } = useAuth();
  const { handleGoogleAuth, isGoogleLoading, googleError } = useGoogleAuth();
    ...
    <div className="flex flex-col flex-1">
      ...
      <div className="flex flex-col justify-center flex-1 w-full max-w-md mx-auto">
        <div>
          ...
          <div>
            {googleError && (
              <div className="mb-4 p-3 text-sm text-error-500 bg-error-50 rounded-lg dark:bg-error-950/30 dark:border dark:border-error-800">
                {googleError}
              </div>
            )}
            <div className="grid grid-cols-1 gap-3 sm:gap-5">
              <button
                ...
                className="inline-flex w-full items-center justify-center gap-3 py-3 text-sm font-normal text-gray-700 transition-colors bg-gray-100 rounded-lg px-7 hover:bg-gray-200 hover:text-gray-800 disabled:opacity-50 disabled:cursor-not-allowed dark:bg-white/5 dark:text-white/90 dark:hover:bg-white/10"
              >
                {isGoogleLoading ? (
            ...
            <form onSubmit={handleLogin}>
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <Input value={email} placeholder="info@gmail.com" onChange={(e) => setEmail(e.target.value)} />
                </div>
                <div>
                  <Label>
